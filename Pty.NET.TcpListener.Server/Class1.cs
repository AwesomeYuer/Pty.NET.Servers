...
using System.Text;
using static AutoIt.Common.EncodingDetector;
...
namespace AutoIt.Common
{
    public class EncodingDetector
    {
        ...
        public static int GetBomLengthFromEncodingMode(EncodingIdentity encoding)
        {
            int length;

            switch (encoding)
            {
                case EncodingIdentity.Utf16BeBom:
                case EncodingIdentity.Utf16LeBom:
                    length = 2;
                    break;

                case EncodingIdentity.Utf8Bom:
                    length = 3;
                    break;
        ...
        public EncodingIdentity CheckBom(byte[] buffer, int size)
        {
            // Check for BOM
            if (size >= 2 && buffer[0] == _utf16LeBom[0] && buffer[1] == _utf16LeBom[1])
            {
                return EncodingIdentity.Utf16LeBom;
            }

            if (size >= 2 && buffer[0] == _utf16BeBom[0] && buffer[1] == _utf16BeBom[1])
            {
                return EncodingIdentity.Utf16BeBom;
            }

            if (size >= 3 && buffer[0] == _utf8Bom[0] && buffer[1] == _utf8Bom[1] && buffer[2] == _utf8Bom[2])
            {
                return EncodingIdentity.Utf8Bom;
            }

            return EncodingIdentity.None;
        }


        private Encoding DetectEncoding(EncodingIdentity encodingIdentity) =>
            encodingIdentity
                switch
                {
                    EncodingIdentity.Ascii => Encoding.ASCII
                    //, EncodingIdentity.Ansi => Encoding.an
                    , EncodingIdentity.Utf8Bom => Encoding.UTF8
                    , EncodingIdentity.Utf8Nobom => Encoding.UTF8
                    , _ => throw new ArgumentOutOfRangeException(nameof(encodingIdentity), $"Not expected {nameof(EncodingIdentity)} value: {encodingIdentity}")

                };

        public Encoding DetectEncoding(ArraySegment<byte> bytes)
        {
            var buffer = bytes.Array;
            return
                DetectEncoding
                    (
                        DetectEncodingIdentity(buffer!, buffer!.Length)
                    );
        }
        ...
        public EncodingIdentity DetectEncodingIdentity(byte[] buffer)
        {
            return
                DetectEncodingIdentity(buffer, buffer.Length);
        }
        ...
        public EncodingIdentity DetectEncodingIdentity(byte[] buffer, int size)
        {
            // First check if we have a BOM and return that if so
            EncodingIdentity encoding = CheckBom(buffer, size);
            if (encoding != EncodingIdentity.None)
            {
                return encoding;
            }
            ...
            {
                return EncodingIdentity.Ansi;
            }

            // Found a null, return based on the preference in null_suggests_binary_
            return _nullSuggestsBinary ? EncodingIdentity.None : EncodingIdentity.Ansi;
        }
        ...
        private static EncodingIdentity CheckUtf16NewlineChars(byte[] buffer, int size)
        {
            if (size < 2)
            {
                return EncodingIdentity.None;
            ...
            if (leControlChars > 0)
            {
                return EncodingIdentity.Utf16LeNoBom;
            }

            return beControlChars > 0 ? EncodingIdentity.Utf16BeNoBom : EncodingIdentity.None;
        }
        ...
        /// <param name="size">The size of the byte buffer.</param>
        /// <returns>Encoding.none, Encoding.Utf16LeNoBom or Encoding.Utf16BeNoBom.</returns>
        private EncodingIdentity CheckUtf16Ascii(byte[] buffer, int size)
        {
            var numOddNulls = 0;
            var numEvenNulls = 0;
            ...
            {
                return EncodingIdentity.Utf16BeNoBom;
            }
        ...
        /// </returns>
        /// <returns>2</returns>
        private EncodingIdentity CheckUtf8(byte[] buffer, int size)
        {
            ...
            while (pos < size)
            {
                ...
                {
                    return EncodingIdentity.None; // Not utf8
                }
            ...
            return onlySawAsciiRange ? EncodingIdentity.Ascii : EncodingIdentity.Utf8Nobom;
        }
    }
}